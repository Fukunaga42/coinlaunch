...
import { getTokenInfoAndTransactions, getTokenInfoFromSubgraph, getTokenUSDPriceHistory, getTokenHolders, getTokenLiquidityEvents } from '@/utils/api';
import { fetchTokenTransactions } from '@/utils/graphql';
  ...
  const fetchTokenData = useCallback(
    async (page: number) => {
      ...
      try {
        ...
        console.log("fetch token info from subgraph for address:", address);
        const tokenData = await getTokenInfoFromSubgraph(address as string);
        console.log("fetched token data from subgraph:", tokenData);
        ...
        setTotalTransactionPages(transactionData.pagination.totalPages);
        setTransactionFetchError(null);
      } catch (error) {
