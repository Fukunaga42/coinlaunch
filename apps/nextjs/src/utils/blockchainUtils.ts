import { formatUnits, parseUnits, maxUint256, decodeEventLog, Log, TransactionReceipt, UserRejectedRequestError, parseEther } from 'viem';
import { useReadContract, useWriteContract, useBalance, useWaitForTransactionReceipt, usePublicClient, useAccount, useContractRead, useSendTransaction } from 'wagmi';
...
export const getBondingCurveAddress = (tokenAddress: `0x${string}` | null): `0x${string}` => {
  if (!tokenAddress) {
    return process.env.NEXT_PUBLIC_SEPOLIA_BONDING_CURVE_MANAGER_ADDRESS as `0x${string}`;
  }
  ...
  const oldAddress = process.env.NEXT_PUBLIC_SEPOLIA_BONDING_CURVE_MANAGER_ADDRESS_OLD as `0x${string}`;
  const oldAddress1 = process.env.NEXT_PUBLIC_SEPOLIA_BONDING_CURVE_MANAGER_ADDRESS_OLD1 as `0x${string}`;
  const newAddress = process.env.NEXT_PUBLIC_SEPOLIA_BONDING_CURVE_MANAGER_ADDRESS as `0x${string}`;
...
export function useCreateToken() {
  ...
  const createToken = async (name: string, symbol: string, initialPurchaseAmount: bigint) => {
    ...
    console.log('Creating token with name:', name, 'and symbol:', symbol, "address:", address);

    try {
      console.log('writeContract Initiating token creation transaction...');
      const totalValue = initialPurchaseAmount;
      console.log('Total value for token creation:', publicClient);
      // // First estimate the gas
      const estimatedGas = await publicClient.estimateContractGas({
        address: process.env.NEXT_PUBLIC_SEPOLIA_BONDING_CURVE_MANAGER_ADDRESS as `0x${string}`,
        abi: BondingCurveManagerABI,
        functionName: 'create',
        args: [name, symbol],
        value: totalValue,
      ...
      let hash;
      try {
        // TEMPORARY TEST: Replace contract call with simple ETH transfer to test MetaMask connectivity
        // console.log('🧪 TEST MODE: Sending simple ETH transfer instead of contract call');
        // console.log('🧪 Sending 0.001 ETH to your own address to test MetaMask popup');
        
        // hash = await sendTransactionAsync({
        //   to: address, // Send to your own address
        //   value: parseEther('0.001'), // Small test amount - 0.001 ETH
        // });

        // COMMENTED OUT: Original contract call that was timing out
        ...
        throw new Error('Error sending transaction: ' + (err instanceof Error ? err.message : String(err)));
      }
      ...
      // Find all logs from our contract
      const contractLogs = receipt.logs.filter(log =>
        log.address.toLowerCase() === (process.env.NEXT_PUBLIC_SEPOLIA_BONDING_CURVE_MANAGER_ADDRESS as `0x${string}`).toLowerCase()
      );
      
      console.log('Contract logs found:', contractLogs.length);

      // Try to decode each log to find TokenCreated event
      ...
      for (const log of contractLogs) {
        try {
          ...
          if (decodedLog.eventName === 'TokenCreated' && decodedLog.args) {
            console.log('TokenCreated event found!');
            const args = decodedLog.args as unknown as { tokenAddress: `0x${string}`; creator: `0x${string}`; name: string; symbol: string };
            tokenAddress = args.tokenAddress;
            console.log('Token created successfully. Address:', tokenAddress);
            break;
          }
        } catch (decodeError) {
