        ...
        {transactions.map((tx) => (
          <tr key={tx.id} className="border-b border-[var(--card-hover)]">
            <td className="px-4 py-2">
              <a 
                href={`https://eth-sepolia.blockscout.com/address/${tx.senderAddress}`}
                target="_blank"
            ...
            <td className="px-4 py-2">
              <a
                href={`https://eth-sepolia.blockscout.com/tx/${tx.txHash}`}
                target="_blank"
                rel="noopener noreferrer"
                className="text-gray-400 hover:text-[var(--primary)] text-sm transition-colors"
              >
    ...
    <div className="w-full">
      {isLoading ? (
        ...
        <div className="text-center py-8">
          ...
          <button
            onClick={() => handlePageChange(transactionPage)}
            className="px-4 py-2 bg-[var(--primary)] text-black rounded-lg text-sm hover:bg-[var(--primary-hover)] transition-colors"
          >
            Retry
          </button>
        </div>
